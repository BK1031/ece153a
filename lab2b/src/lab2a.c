...
QState Lab2A_stateA(Lab2A *me) {
	switch (Q_SIG(me)) {
		...
		case BTNU: { // UP
			xil_printf("Up Button from State A\n");
			OnScreenText("MODE: 1");
			return Q_HANDLED();
		}
		case BTND: { // DOWN
			xil_printf("Down Button from State A\n");
			OnScreenText("MODE: 2");
			return Q_HANDLED();
		}
		case BTNC: { // CENTER
			xil_printf("Center Button from State A\n");
			OnScreenText("MODE: 3");
			return Q_HANDLED();
		}
		case BTNL: { // LEFT
			xil_printf("Left Button from State A\n");
			OnScreenText("MODE: 4");
			return Q_HANDLED();
		}
		case BTNR: { // RIGHT
			xil_printf("Right Button from State A\n");
			OnScreenText("MODE: 5");
			return Q_HANDLED();
		}
